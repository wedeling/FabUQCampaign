...
+ In the examples folder the script `examples/ocean_2D/fab_ocean.py` runs an EasyVVUQ Stochastic Collocation (SC) campaign using FabSim3 for a 2D ocean model on the localhost. 
+ `FabUQCampaign/template/run_UQ_sample` file contains the command line instruction to run a single sample, in this case: `python3 $ocean_exec ocean_in.json`. Here, `ocean_in.json` is just the input file with the parameter values generated by EasyVVUQ. Furthermore, `$ocean_exec` is the full path to the Python script which runs the governing equations at the parameters of `ocean_in.json`. It must be specified in `deploy/machines_user.yml`, which in this case looks like
